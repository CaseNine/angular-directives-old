# Pagination Table directive

## Usage
```html
<pagination-table items="items" show="show"></pagination-table>
```

### @items en @show
Attribute om met de parent controller te communiceren
...
## @show
Json met de getoonde value in de table;

Key: en Value zijn verplicht!;

Key voor de naam boven de table denk aan vertaling;
Value voor de attribute in de @items array

```javascript
	$scope.show = [
	              {"key": "id", "value": "id"},
	              {"key": "Omschrijving", "value": "description"}
	 ];
```
...
## Output
Table with pagination en sorting on de page
